    ...
    with db_lock:
        with _connect_db() as conn:
            conn.execute(
                """
                INSERT INTO jobs (
                    job_id, url, quality, output_path, playlist_mode, playlist_items,
                    download_subtitles, subtitle_languages, save_thumbnail_only,
                    status, logs, result, error, created_at, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    job["job_id"],
                    ...
                    job["quality"],
                    job["output_path"],
                    int(job["playlist_mode"]),
                    _json_dump(job["playlist_items"]),
                    int(job["download_subtitles"]),
                    _json_dump(job["subtitle_languages"]),
                    int(job["save_thumbnail_only"]),
                    job["status"],
                    _json_dump(job["logs"]),
                    _json_dump(job["result"]),
                    job["error"],
                    job["created_at"],
                    job["updated_at"],
                ),
...
def list_jobs():
    ...
    try:
        ...
        limit = 50
    return jsonify({"jobs": _list_jobs(limit)})
